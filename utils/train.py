    ...
    def run(self, dataloader):
        ...
        with tqdm(dataloader, desc=self.stage_name, file=sys.stdout, disable=not (self.verbose)) as iterator:
            for x, y, filename in iterator:
                ...
                #print(y_pred.size())
                ## 该语句y_pred=torch.argmax(y_pred,dim=-3)是后填加的，其对效果影响很大
                #y_pred = torch.argmax(y_pred, dim=-3)
                #y_pred = torch.softmax(y_pred, dim=-3)
                #print(y_pred.size())

                #这是我加的,将model最后一层Logsoftmax变成 softmax输出, 目的是为了计算metrics
        ...
        # 这里在logs中保存了每个category的confusion matrix，为了方便后续计算每个类的metric
